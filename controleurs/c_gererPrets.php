...
switch ($action) {
            ...
            case 'validerPret' : {
                if (isset($_POST["cmdValider"])) {
                    ...
                    if (!empty($strClient) and ! empty($strOuvrage) and ! empty($strDate_emprunt)) {
                        // tests de cohérence et de la date d'acquisition
                    ...
                    if (!$hasErrors) {
                        try {
                            ...
                            if ($res > 0) {
                                ...
                                // récupération des prêts correspondants au numéro
    ...
    case 'modifierPret' : {
        ...
        // On ne rentre dans le switch que si l'id est transmis et l'id de pret existe
        if (!$hasErrors) {
            switch ($option) {
                case 'saisirPret' : {
                        $lesClients = ClientDal::loadClients(0);
